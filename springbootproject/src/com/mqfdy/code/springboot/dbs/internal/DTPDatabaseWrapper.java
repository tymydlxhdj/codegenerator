	...
	implements InternalDatabase
{
	...
	// lazy-initialized
	private DTPCatalogWrapper[] dCatalogs;
	...
	private synchronized DTPCatalogWrapper[] catalogs_() {
		if (this.dCatalogs == null) {
			this.dCatalogs = this.buildCatalogs();
		}
	...
	private DTPCatalogWrapper buildDefaultCatalog() {
		...
		for (Iterator<DTPCatalogWrapper> stream = this.catalogWrappers(); stream.hasNext(); ) {
			...
			if (catalog.getName().equalsIgnoreCase(this.getName())) {
	...
	private void disposeCatalogs() {
		if (this.dCatalogs != null) {
			for (DTPCatalogWrapper catalog : this.dCatalogs) {
				catalog.dispose();
			}
			this.dCatalogs = null;
		}
