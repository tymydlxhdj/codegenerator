...
/** Keep track of defined macros, used for escape processing. */
    ...
    /**
	 * This constructor was added to allow the re-use of parsers. The normal
	 * constructor takes a single argument which an InputStream. This simply
	 * creates a re-usable parser object, we satisfy the requirement of an
	 * InputStream by using a newline character as an input stream.
	 *
	 * @param rs
	 *            the rs
	 */
    ...
    /**
	 * This was also added to allow parsers to be re-usable. Normal JavaCC use
	 * entails passing an input stream to the constructor and the parsing
	 * process is carried out once. We want to be able to re-use parsers: we do
	 * this by adding this method and re-initializing the lexer with the new
    ...
    /**
	 * Produces a processed output for an escaped control or pluggable
	 * directive.
	 *
	 * @author mqfdy
	 * @param strImage
	 ...
	 * whitespaces. This method is used in the semantic look ahead of Directive
	 * method. This is done in code instead of as a production for simplicity
	 * and efficiency.
	 *
	 * @author mqfdy
...
/**
 * This method is what starts the whole parsing process. After the parsing is
 * complete and the template has been turned into an AST, this method returns
 * the root of AST which can subsequently be traversed by a visitor which
 * implements the ParserVisitor interface which is generated automatically by
 * JavaCC
 *
 * @author mqfdy
 * @return the simple node
...
/**
 * used to separate the notion of a valid directive that has been escaped,
 * versus something that looks like a directive and is just schmoo. This is
 * important to do as a separate production that creates a node, because we want
 * this, in either case, to stop the further parsing of the Directive() tree.
 *
 * @author mqfdy
 * @throws ParseException
 *             the parse exception
...
/**
 * Used to catch and process escape sequences in grammatical constructs as
 * escapes outside of VTL are just characters. Right now we have both this and
 * the EscapeDirective() construction because in the EscapeDirective() case, we
 * want to suck in the #<directive> and here we don't. We just want the escapes
 * to render correctly
 *
 * @author mqfdy
 * @throws ParseException
...
/**
 * This method corresponds to variable references in Velocity templates. The
 * following are examples of variable references that may be found in a
 * template:
 * 
 * $foo $bar
 *
...
/**
 * supports the [n..m] vector generator for use in the #foreach() to generate
 * measured ranges w/o needing explicit support from the app/servlet
 *
 * @author mqfdy
 * @throws ParseException
 *             the parse exception
...
/**
 * This method has yet to be fully implemented but will allow arbitrarily nested
 * method calls.
 *
 * @author mqfdy
 * @throws ParseException
...
/**
 * This method is responsible for allowing all non-grammar text to pass through
 * unscathed.
 *
 * @author mqfdy
 * @throws ParseException
