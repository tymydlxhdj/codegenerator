...
/**
 * Base class which simplifies the use of Velocity with Servlets. Extend this
 * class, implement the <code>handleRequest()</code> method, and add your data
 * to the context. Then call <code>getTemplate("myTemplate.wm")</code>.
 * 
 * This class puts some things into the context object that you should be aware
 * of:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * There are other methods you can override to access, alter or control any part
 * of the request processing chain. Please see the javadocs for more information
 * on :
 * <ul>
 * <li>loadConfiguration() : for setting up the Velocity runtime
 * <li>createContext() : for creating and loading the Context
 * <li>setContentType() : for changing the content type on a request by request
 * basis
 ...
 * </ul>
 * <br>
 * If you put a String with key "contentType" object into the context within
 * either your servlet or within your template, then that will be used to
 * override the default content type specified in the properties file.
 *
 * @author Dave Bryson
 * @author <a href="mailto:jon@latchkey.com">Jon S. Stevens</a>
 * @author <a href="mailto:geirm@optonline.net">Geir Magnusson Jr.</a>
 * @author <a href="kjohnson@transparent.com">Kent Johnson</a>
 * @author <a href="dlr@finemaltcoding.com">Daniel Rall</a> $Id:
 *         VelocityServlet.java 463298 2006-10-12 16:10:32Z henning $
 * @deprecated This servlet has been replaced by VelocityViewServlet, available
 *             from the Velocity-Tools sub-project. VelocityViewServlet provides
 *             support for quick, clean MVC web development. VelocityServlet
 *             will be removed in a future version of Velocity.
 */
public abstract class VelocityServlet extends HttpServlet
{
    ...
    /**
	 * Performs initialization of this servlet. Called by the servlet container
	 * on loading.
	 *
	 * @author mqfdy
	 * @param config
	 ...
	 * loadConfiguration(ServletConvig) to get a java.util.Properties of
	 * configuration information and then calling Velocity.init(). Override this
	 * to do anything to the environment before the initialization of the
	 * singelton takes place, or to initialize the singleton in other ways.
	 *
	 * @author mqfdy
	 * @param config
	 *            the config
    ...
    /**
	 * Loads the configuration information and returns that information as a
	 * Properties, which will be used to initialize the Velocity runtime. <br>
	 * <br>
	 * Currently, this method gets the initialization parameter
	 * VelocityServlet.INIT_PROPS_KEY, which should be a file containing the
	 * configuration information. <br>
	 * <br>
	 * To configure your Servlet Spec 2.2 compliant servlet runner to pass this
	 * to you, put the following in your WEB-INF/web.xml file <br>
	 * 
	 * <pre>
	 *    &lt;servlet&gt;
	 ...
	 * </pre>
	 * 
	 * Alternately, if you wish to configure an entire context in this fashion,
	 ...
	 * </pre>
	 * 
	 * and then add or modify the configuration values from the file. <br>
	 *
	 * @param config
	 *            ServletConfig passed to the servlets init() function Can be
	 *            used to access the real path via ServletContext (hint)
	 ...
	 * @deprecated Use VelocityViewServlet from the Velocity Tools library
	 *             instead.
	 */
    ...
    /**
	 * Handles HTTP <code>GET</code> requests by calling
	 * {@link #doRequest(HttpServletRequest, HttpServletResponse)}.
	 *
	 * @author mqfdy
	 * @param request
	 *            the request
    ...
    /**
	 * Handles HTTP <code>POST</code> requests by calling
	 * {@link #doRequest(HttpServletRequest, HttpServletResponse)}.
	 *
	 * @author mqfdy
	 * @param request
	 ...
	 * {@link #doRequest(HttpServletRequest, HttpServletResponse)} processing
	 * sequence, allowing a derived class to do resource cleanup or other end of
	 * process cycle tasks.
	 *
	 * @author mqfdy
	 * @param request
	 ...
	 * {@link #chooseCharacterEncoding(HttpServletRequest)} to select the
	 ...
	 * {@link #setContentType(HttpServletRequest, HttpServletResponse)} if an
	 ...
	 * value. <br>
	 * <br>
	 * In the event of a problem, you may handle the request directly and return
	 * <code>null</code> or throw a more meaningful exception for the error
	 * handler to catch.
	 *
	 * @param request
	 *            servlet request from client
	 ...
	 * @return The template to merge with your context or null, indicating that
	 *         you handled the processing.
	 * @throws Exception
	 *             the exception
	 * @since Velocity v1.1
	 */
	 ...
	 * value. <br>
	 * <br>
	 * In the event of a problem, you may simple return <code>null</code> or
	 ...
	 *             the exception
	 * @deprecated Use
	 *             {@link #handleRequest( HttpServletRequest request, HttpServletResponse response, Context ctx )}
	 */
