...
public class ReverseContants {
	...
	public final static class Oracle_SQL {
		...
		public static String getUniqueColumnsSQL(String dbType) {
			...
			/*if (DBType.KingBase.getDbType().equals(dbType)) {
				buffer.append("select col.column_name column_name from   ");
				buffer.append(" user_constraints au, all_objects obj ,user_ind_columns col");
				buffer.append(" where au.constraint_name=obj.object_name ");
				buffer.append(" and au.constraint_type = 'U' and col.index_name = obj.object_name   and au.table_name = ?");
			} else */if(DBType.Oracle.getDbType().equals(dbType)){
