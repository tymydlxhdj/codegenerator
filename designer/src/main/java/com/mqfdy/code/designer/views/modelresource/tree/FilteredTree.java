...
//	private static final String DISABLED_CLEAR_ICON = "org.eclipse.ui.internal.dialogs.DCLEAR_ICON"; //$NON-NLS-1$
			...
			public IStatus runInUIThread(IProgressMonitor monitor) {
				...
				try {
					...
					if (getFilterString().equals("")) {
						Object ab = BusinessModelUtil
								.getBusinessModelDiagramEditor().getViewer()
								.getContents().getModel();
						TreeItem[] items1 = treeViewer.getTree().getItems();
						treeViewer.expandToLevel(ab,
								AbstractTreeViewer.ALL_LEVELS); /* 使需要的模型可见 */
						// TreeItem item =(TreeItem)
						// treeViewer.testFindItem(ab);
						// treeViewer.getTree().setSelection(item);/*使模型在视图上变成可见状态*/
						TreeItem item = getMatchingItem(items1, ab);
						if (item != null) {
	...
	private void createClearTextOld(Composite parent) {
		...
		if ((filterText.getStyle() & SWT.ICON_CANCEL) == 0) {
			...
			clearTextAction.setImageDescriptor(BusinessModelEditorPlugin.
					getImageDescriptor(ImageKeys.IMG_OUTLINE_CLEAR_ICON));
			clearTextAction.setDisabledImageDescriptor(BusinessModelEditorPlugin
	...
	private void createClearTextNew(Composite parent) {
		...
		// natively
		if ((filterText.getStyle() & SWT.ICON_CANCEL) == 0) {
			final Image inactiveImage =  BusinessModelEditorPlugin.
					getImageDescriptor(ImageKeys.IMG_OUTLINE_DISABLED_CLEAR_ICON).createImage();
			final Image activeImage = BusinessModelEditorPlugin.
					getImageDescriptor(ImageKeys.IMG_OUTLINE_CLEAR_ICON).createImage();
	...
	private TreeItem getMatchingItem(TreeItem[] items, Object ab) {
		for (int i = 0; i < items.length; i++) {
			if (ab == items[i].getData()) {
				return items[i];
			}
			TreeItem treeItem = getMatchingItem(items[i].getItems(), ab);
			if (treeItem != null) {
				return treeItem;
			}
		}
