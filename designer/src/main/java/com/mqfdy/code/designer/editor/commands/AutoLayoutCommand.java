...
public class AutoLayoutCommand extends Command {
	...
	public void execute() {
		if (canExecute()) {
			Shell shell = BusinessModelEditorPlugin.getActiveWorkbenchWindow()
					.getShell();
			AutoLayoutDiaolg dialog = new AutoLayoutDiaolg(shell);
	...
	private void getGraphsRecursively(NodeEditPart rootEntity,
		...
		for(OmConnectionEditPart rel : connEditList2){
			if(!graphs.contains(rel.getTarget()) && rel.getSource().equals(rootEntity)){
			...
			}else if(!graphs.contains(rel.getSource()) && rel.getTarget().equals(rootEntity)){
...
//					omGraphs[i].setLayoutAlgorithm(new CompositeLayoutAlgorithm(
//							layoutStyle, new LayoutAlgorithm[] {
//									new GridLayoutAlgorithm(layoutStyle),
//									new HorizontalShift(layoutStyle),
//									new VHShift(layoutStyle),
//									new ShiftDiagramLayoutAlgorithm(layoutStyle) 
//									}), true);
...
//					omGraphs[i].setLayoutAlgorithm(new CompositeLayoutAlgorithm(
//							layoutStyle, new LayoutAlgorithm[] {
//									new SpringLayoutAlgorithm(layoutStyle),
//									new HorizontalShift(layoutStyle),
//									new VHShift(layoutStyle),
//									new ShiftDiagramLayoutAlgorithm(layoutStyle) 
//									}), true);
